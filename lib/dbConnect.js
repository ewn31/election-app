...
async function updateStudent(data) {
    ...
    try {
        await db.execute(
            'UPDATE `STUDENTS` SET MATRICULE=?, NAME=?, FACULTY=?, DEPARTMENT=?, HASHED_PASSWORD=? WHERE MATRICULE=?;',
            [matricule, name, faculty, dpt, hash, matricule] 
        )
    } catch (error) {
        throw new Error('DB Error', {cause: error})
    }
...
async function getElection(id) {
    try {
        const result = await db.query('SELECT * FROM ELECTIONS WHERE ID = ?;',
            [id])
            return result[0][0];
    } catch (error) {
        console.log(error);
...
async function updateElection(data,id) {
    const n = data.name;
    const type = data.type;
    ...
    try {
        db.query('UPDATE ELECTIONS SET NAME=?, TYPE=?, SCOPE=?, START_DATE=?, END_DATE=? WHERE ID=?;',
            [n, type, scope, start_date, end_date, id]);
        return true;
    } catch (error) {
        console.log(error);
        return false;
...
async function deleteElection(id) {
    try {
         await db.query('DELETE FROM ELECTIONS WHERE ID=?;',
            [id]);
        return true
    } catch (error) {
        console.log(error)
...
async function getAuthInfo (user){
   try {
    const result = await db.query('select * from AUTHTOKEN where USER=?', [user]);
    console.log(result);
...
async function addAuthinfo(data) {
    const token = data.token;
    const user = data.user;
    console.log("In add addAuthinfo");
    try {
        await db.query("INSERT INTO AUTHTOKEN VALUES (?, ?);",
            [user, token])
    } catch (error) {
        console.log(error);
    }
...
async function deleteAuthinfo(user) {
    await db.query('DELETE FROM AUTHTOKEN WHERE ID = ?;',[user]);
}
module.exports = {getCity, addStudent, updateStudent, verifyUser, verifyAdmin, addElection, getElections, getElection,updateElection, deleteElection, addCandidate, getStudent, getStudentElections, getCandidates, voteCandidates, getElectionStatus, registerVote, getHistory, addAuthinfo, getAuthInfo, deleteAuthinfo};
